...
// 获取API基础URL，优先使用环境变量，否则使用空字符串（通过Vite代理）
export const getApiBaseUrl = () => {
  ...
  // 开发环境返回空字符串，请求会通过Vite代理转发到后端
  // 生产环境若无VITE_API_BASE_URL也返回空字符串
  return "";
};
...
export async function apiRequest(
  ...
  console.log(
    `发送请求到: ${fullUrl}`,
    proxiedUrl !== fullUrl ? `(通过代理: ${proxiedUrl})` : "",
  );
    ...
    try {
      ...
      if (res.status === 503) {
        if (attempt < maxRetries) {
          ...
          console.warn(
            `服务不可用(503)，正在第${attempt + 1}次重试(共${maxRetries}次)...`,
          );
      ...
      // 否则等待1秒后重试
      console.warn(
        `请求失败，正在第${attempt + 1}次重试(共${maxRetries}次)...错误：${lastError.message}`,
      );
      await sleep(1000);
    }
  }
